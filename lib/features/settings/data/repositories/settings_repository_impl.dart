...
class SettingsRepositoryImpl implements SettingsRepository {
  ...
  Future<void> importJson(String payload) async {
    ...
    final categoriesJson = (data['categories'] as List?)?.cast<Map<String, dynamic>>() ?? const [];
    final transactionsJson = (data['transactions'] as List?)?.cast<Map<String, dynamic>>() ?? const [];
    ...
    await _ref.read(appDatabaseProvider).replaceAllData(
  ...
  Stream<SettingsEntity> watchSettings() {
    return _ref.read(appDatabaseProvider).watchSettingsRow().map((row) {
      if (row == null) {
        return SettingsEntity(updatedAt: DateTime.now(), themeMode: AppThemeMode.system);
      }
      return row.toEntity();
    });
  }
